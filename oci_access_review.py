...
Runs on the 2nd of January, April, July, October.

1. Exports OCI IAM user list to CSV
2. Ensures the configured epic "OCI User Access Review and Cleanup" exists (creates once, reuses forever)
3. Creates a Jira task under that epic, assigned to user-erp@example.com
4. Attaches the CSV to the Jira ticket
5. Posts to Slack #oci-admin with @mention and CSV file attachment

Run modes (--mode or RUN_MODE env var):
  check  : run only if today is the 2nd of a quarter month (default)
...
ASSIGNEE_ACCOUNT_ID = os.environ.get("OCI_ADMIN_ASSIGNEE_ACCOUNT_ID", "")
ASSIGNEE_EMAIL      = os.environ.get("OCI_ADMIN_ASSIGNEE_EMAIL", "")

IDCS_ENDPOINT  = os.environ.get("OCI_ADMIN_IDCS_ENDPOINT", "")  # e.g. https://idcs-XXXX.identity.oraclecloud.com
...
def is_quarter_start(today: date = None) -> bool:
    ...
    return d.day == 2 and d.month in QUARTER_MONTHS
...
def main():
    ...
    if args.mode == "check" and not is_quarter_start(today):
        print("Not the 2nd of a quarter month — nothing to do.")
        return
    ...
    try:
        ...
        mention       = slack_mention(ASSIGNEE_EMAIL)
