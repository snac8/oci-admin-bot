...
Required environment variables (set in /etc/oci-admin/secrets.env):
  SLACK_BOT_TOKEN       - Slack bot token (xoxb-...)
  SLACK_CHANNEL         - Slack channel (default: #oci-admin)
...
SAAS_USAGE_DIR    = os.environ.get("OCI_ADMIN_SAAS_USAGE_DIR")
JIRA_BASE         = "https://your-org.atlassian.net"
JIRA_EMAIL        = os.environ.get("OCI_ADMIN_JIRA_EMAIL", "your-email@example.com")
JIRA_TOKEN        = os.environ.get("OCI_ADMIN_JIRA_TOKEN", "")
JIRA_PROJECT      = os.environ.get("OCI_ADMIN_JIRA_PROJECT", "")
JIRA_ISSUE_TYPE   = "10005"   # Task
JIRA_PRIORITY     = "10004"   # P2 (Important)
JIRA_COMPONENT    = os.environ.get("OCI_ADMIN_JIRA_COMPONENT", "")  # Jira component ID
JIRA_EPIC         = "<your-saas-usage-epic-key>"  # Oracle SaaS Service Usage Review and Cleanup
JIRA_SPRINT       = int(os.environ.get("OCI_ADMIN_JIRA_SPRINT", "0"))  # optional, sprint board ID
...
SLACK_USER_ERP      = os.environ.get("SLACK_USER_ERP",      "your-email@example.com")
SLACK_USER_EPBCS    = os.environ.get("SLACK_USER_EPBCS",    "your-email@example.com")
SLACK_USER_FCCS_EDM = os.environ.get("SLACK_USER_FCCS_EDM", "your-email@example.com")
...
def _shorten_epm_name(raw):
    instance_prefix = os.environ.get("OCI_ADMIN_REPORT_INSTANCE_PREFIX", "")
    lines = [
        l.strip() for l in raw.split("\n")
        if l.strip() and not (instance_prefix and l.strip().startswith(instance_prefix))
    ]
    name = " - ".join(lines[:2])
                ...
                # --- Detailed per-instance user table (pages 6-7) ---
                    ...
                    for row in table[1:]:
                        ...
                        if not part or not service or str(part).startswith("Total"):
                            continue
                        # Extract instance ID: last line matching the report instance prefix
                        instance_prefix = os.environ.get("OCI_ADMIN_REPORT_INSTANCE_PREFIX", "")
                        lines = [l.strip() for l in str(service).split("\n") if l.strip()]
                        if instance_prefix:
                            instance = next((l for l in reversed(lines) if l.startswith(instance_prefix)), lines[-1])
                        else:
                            instance = lines[-1]
                        try:
...
if __name__ == "__main__":
    if not SLACK_BOT_TOKEN:
        print("ERROR: OCI_ADMIN_SLACK_BOT_TOKEN is not set")
        sys.exit(1)
    if not JIRA_TOKEN:
        print("ERROR: OCI_ADMIN_JIRA_TOKEN is not set")
        sys.exit(1)
    if not SAAS_USAGE_DIR:
        print("ERROR: OCI_ADMIN_SAAS_USAGE_DIR is not set")
        sys.exit(1)
    ...
    post_combined_slack(
        ...
        epm_file, epm_months, epbcs_services, fccs_edm_services,
        epbcs_detail, fccs_detail, employee,
        erp_key or "<missing-jira-key>", epbcs_key or "<missing-jira-key>", fccs_edm_key or "<missing-jira-key>",
    )
