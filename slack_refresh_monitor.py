...
Supported channel command:
  @oci-admin refresh <url> on <date> [<time>] [PT]
  @oci-admin when is the next refresh of dev1?
  @oci-admin what's scheduled for dev2?

Supported thread commands (must @mention bot):
  @oci-admin cancel
  @oci-admin reschedule to <date> <time> PT
  @oci-admin cancel and reschedule to <date> <time> PT
  @oci-admin status / when is this / what's scheduled
"""
...
SLACK_CHANNEL_ID = os.environ.get("OCI_ADMIN_SLACK_CHANNEL_ID", "<your-slack-channel-id>")
PROD_OCID = os.environ.get("PROD_OCID", "")

# Map keyword → (ocid, display_url)
# Each maps to the Oracle Fusion environment hostname configured in your tenancy.
KNOWN_ENVIRONMENTS = {
    "dev1": (
        os.environ.get("DEV1_OCID", ""),
        "https://your-dev1-tenant.oraclecloud.com/",
    ),
    "dev2": (
        os.environ.get("DEV2_OCID", ""),
        "https://your-dev2-tenant.oraclecloud.com/",
    ),
    "dev3": (
        os.environ.get("DEV3_OCID", ""),
        "https://your-dev3-tenant.oraclecloud.com/",
    ),
    "dev4": (
        os.environ.get("DEV4_OCID", ""),
        "https://your-dev4-tenant.oraclecloud.com/",
    ),
    "test": (
        os.environ.get("TEST_OCID", ""),
...
def parse_channel_command(text: str) -> dict | None:
    ...
    noise = (r'\b(oci.?admin|refresh|schedule[d]?|restart|please|a|an|the|'
...
def handle_channel_command(msg: dict, bot_mention: str, channel_id: str) -> dict | None:
    ...
    if not any(kw in text_lower for kw in ("refresh", "schedule", "restart")):
        post_thread_reply(channel_id, msg_ts,
            ...
            "• `@oci-admin refresh <url> on July 20 3:00 PM PT` — schedule a refresh\n"
            "• `@oci-admin when is the next refresh of dev2?` — check scheduled refreshes\n"
            "• `@oci-admin status dev1` — show all scheduled refreshes for dev1")
        return None
    ...
    if not parsed:
        post_thread_reply(channel_id, msg_ts,
            ...
            "`@oci-admin refresh <url> on July 20 3:00 PM PT`\n"
    ...
    post_thread_reply(channel_id, msg_ts,
        ...
        f"To cancel or reschedule, reply `@oci-admin cancel` or `@oci-admin reschedule to <date> <time> PT`.")
...
def handle_reschedule(channel_id: str, thread_ts: str, state: dict, text: str) -> dict | None:
    ...
    if not new_scheduled_start:
        post_thread_reply(channel_id, thread_ts,
            ...
            "• `@oci-admin reschedule to July 18 4pm PT`\n"
            "• `@oci-admin reschedule it for July 18 4pm PT`\n"
            "• `@oci-admin reschedule for July 18` _(defaults to 5:00 PM PT)_")
        return None
...
def main():
    ...
    # ------------------------------------------------------------------
    last_channel_ts = state.get("last_channel_ts") or str(time.time())
            ...
            for msg in replies:
                ...
                if is_status_query(text):
                    ...
                    post_thread_reply(t_channel, t_ts,
                        ...
                        "• `@oci-admin cancel`\n"
                        "• `@oci-admin reschedule to <date> <time> PT`\n"
                        "• `@oci-admin when is the next refresh?` — check scheduled refreshes\n"
